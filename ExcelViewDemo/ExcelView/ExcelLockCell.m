...
static NSString * const AddNewCellID = @"AddNewCellID";

@interface ExcelLockCell()<deleteHomeThrid>
...
//        [self.mColumeMaxWidths[1] floatValue]
...
//
//        }
        HomeTableCell *cell = [[HomeTableCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:AddNewCellID];

        [cell GetHomeTableCellData :nil cellNumber:(int)indexPath.row widthList:[_mColumeMaxWidths copy]];
        
        cell.delegatehomethrid  = self;
        return cell;
    }
    return nil;
}
...
{
    if([self.delegatehomeSecond  respondsToSelector:@selector(touchhomeSecond:cellNumber:didSelectItemAtIndex:)])
    {
        [self.delegatehomeSecond touchhomeSecond:self cellNumber:CellNumber didSelectItemAtIndex:index];
    }
}
